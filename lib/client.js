 ...
 *   {boolean} options.gzip             - (optional) - GZip compression, default false.
 *   {boolean} options.deflate          - (optional) - Deflate compression, default false.
...
function Client(options, isSecure) {
  ...
  options.deflate   = options.deflate || false;
  ...
  var compressions = [];
  if (options.gzip) {
    compressions.push('gzip');
  }
  if (options.deflate) {
    compressions.push('deflate');
  }
  if (compressions.length > 0) {
    headers['Accept-Encoding'] = compressions.join(',');
  }
...
Client.prototype.methodCall = function methodCall(method, params, callback) {
  ...
  /*if (options.keepAlive) {
    request.timeout(options.keepAlive);
  }*/
